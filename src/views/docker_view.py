...
docker = [
    ...
    ("Build an image from a Dockerfile", "docker build -t <image_name>:<version> .  OR docker build -t <image_name>:<version> . -no-cache"),
    # CONTAINER
    ...
    ("Open shell inside running container", "docker exec -it <container_name> /bin/bash OR docker exec -it <container_name> sh"),
    # DOCKERHUB
    ("Pull an image from DockerHub", "docker pull <image_name>"),
    ("Publish an image to DockerHub", "docker push <username>/<image_name>"),
    ("Login into DockerHub", "docker login -u <image_name> OR docker login //also, docker logout to remove credentials"),
    ...
    ("Mount Named volume with running container", "docker run --volume <volume_name>:<mount_path> OR docker run --mount type=volume, src=<volume_name>, dest=<mount_path>"),
    ("Mount Anonymous volume with running container", "docker run - -volume <mount_path>"),
    ("To create a Bind Mount", "docker run - -volume <host_path>:<container_path> OR docker run - -mount type=bind,src=<host_path>,dest=<container_path>"),
