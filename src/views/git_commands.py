...
class GitView(ft.View):
    def __init__(self, page: ft.Page):
        super().__init__()
        self.route="/git", 
        # self.controls=[self.build_content(page)], 
        self.scroll=ft.ScrollMode.ADAPTIVE
        self.page = page

        self.appbar = ft.AppBar(
            title=ft.Text('Git', size=15),
            center_title=False,
            bgcolor='#06b7bd',
        ...
        cards = [self.create_card(i + 1, command, description, example) for i, (command, description, example) in enumerate(git_commands)]
        self.list_view.controls = cards
        self.controls.append(ft.Column(controls=cards))
        page.update()
        # return ft.Column(controls=cards)

    def create_card(self, serial, command, description, example):
        card = ft.Card(
